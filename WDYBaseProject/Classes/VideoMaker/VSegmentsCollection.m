...
-(void)synchronizeTransitions
{
    ...
    for (VTransition* trans in self.transitions) {
        NSLog(@"trans = %@", trans);
    }
    
    for (NSInteger i = 0; i < self.segments.count; i++) {
        VAssetSegment* assetSegment = self.segments[i];
        assetSegment.frontTransition = (i > 0) ? self.transitions[i - 1] : nil;
        assetSegment.rearTransition = (i < self.transitions.count) ? self.transitions[i] : nil;
        
        NSLog(@"frontTransition = %@, rearTransition = %@", assetSegment.frontTransition, assetSegment.rearTransition);
        
        [assetSegment calculateTiming];
    }
}
