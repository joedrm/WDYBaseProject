...
+ (VAsset *)makeFromImageUrl:(NSString *)imageUrl{
    
    VAssetWebImage* newAsset = [VAssetWebImage new];
    newAsset.imageUrlStr = imageUrl;
...
//    if (kObjectIsEmpty(self.dznMetaData)) {
...
{
    [[SDWebImageManager sharedManager] downloadImageWithURL:[self getImageURL] options:(SDWebImageProgressiveDownload | SDWebImageRetryFailed) progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            //do nothing
...
-(void) getPreviewImageForSize: (CGSize) size withCompletion: (VAssetDownloadCompletionBlock) downloadCompletionBlock
{
    ...
    if (self.downloadedImage != nil) {
        ...
        self.currentDownloadingOperation = [[SDWebImageManager sharedManager] downloadImageWithURL:[self getImageURL] options:(SDWebImageProgressiveDownload | SDWebImageRetryFailed) progress:^(NSInteger receivedSize, NSInteger expectedSize) {
